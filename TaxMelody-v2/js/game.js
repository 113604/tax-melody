...
init(){document.addEventListener('keydown',e=>{if(e.key==='Escape'){e.preventDefault();this.togglePause();return}if(e.repeat)return;const lane=this.keyMap[e.key.toLowerCase()];if(lane!==undefined){e.preventDefault();this.judge(lane)}});document.querySelectorAll('.key-btn').forEach(btn=>btn.addEventListener('pointerdown',e=>{e.preventDefault();this.judge(Number(btn.dataset.lane))}));document.getElementById('song').addEventListener('ended',()=>this.finish())},
...
judge(lane){if(!this.state?.started||this.state.ended||this.paused)return;const now=TMAudio.time(),w=TMSettings.values.judgeStrictness==='strict'?.18:TMSettings.values.judgeStrictness==='relaxed'?.3:.24;const n=this.notes.filter(x=>!x.judged&&x.lane===lane&&Math.abs(x.time-now)<=w).sort((a,b)=>Math.abs(a.time-now)-Math.abs(b.time-now))[0];this.pulse(lane);if(!n)return;const d=Math.abs(n.time-now);let result=d<=(w*.38)?'PERFECT':d<=(w*.68)?'GREAT':'GOOD';n.judged=true;n.el?.classList.add('hit');setTimeout(()=>n.el?.remove(),180);if(result==='PERFECT'){this.state.perfect++;this.state.score+=1000;this.state.life=Math.min(1000,this.state.life+8)}if(result==='GREAT'){this.state.great++;this.state.score+=700;this.state.life=Math.min(1000,this.state.life+4)}if(result==='GOOD'){this.state.good++;this.state.score+=400}this.state.combo++;this.state.maxCombo=Math.max(this.state.maxCombo,this.state.combo);this.showJudge(result);this.updateHud()},
